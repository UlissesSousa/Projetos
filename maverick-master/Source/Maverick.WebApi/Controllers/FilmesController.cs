...
namespace Maverick.WebApi.Controllers
{
    ...
    public class FilmesController : ApiController
    {
        ...
        [Route("pessoas")]
        [HttpGet, AllowAnonymous]
        [ProducesResponseType(typeof(PessoasGetResult), 200)]
        [ProducesResponseType(typeof(CoreException<CoreError>), 400)]
        [ProducesResponseType(typeof(InternalError), 500)]
        public async Task<IActionResult> GetPessoasAsync(
    [FromQuery] PessoasGet pessoasGet)
        {
            PesquisaPessoas pesquisa = mapper.Map<PessoasGet, PesquisaPessoas>(pessoasGet);
            IEnumerable<Pessoa> filmes = await filmesService
                .ObterPessoasAsync(pesquisa);
            IEnumerable<PessoasGetResult> PessoasGetResults =
                mapper.Map<IEnumerable<PessoasGetResult>>(filmes);

            return Ok(PessoasGetResults);
        }
