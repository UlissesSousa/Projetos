...
namespace Maverick.TmdbAdapter
{
    ...
    PesquisaPessoas pesquisa, string idioma)
        {
            try
            {
                var cacheKey = $"filmes::{pesquisa.TermoPesquisa}:: {idioma}";

                if (!typedCache.TryGet(cacheKey,
                    out TmdbSearchPeopleGetResult tmdbSearchPeopleGetResult))
                {
                    var tmdbSearchMoviesGet =
                        mapper.Map<TmdbSearchMoviesGet>(pesquisa);
                    ...
                    tmdbSearchPeopleGetResult = await tmdbApi
                        .SearchPeople(tmdbSearchMoviesGet);

                    typedCache.Set(cacheKey, tmdbSearchPeopleGetResult,
                        TimeSpan
                        .FromSeconds(
                            tmdbAdapterConfiguration
                            .TempoDeCacheDaPesquisaEmSegundos));
                ...
                return mapper
                    .Map<IEnumerable<Pessoa>>(tmdbSearchPeopleGetResult.Results);
            }
            catch (ApiException e)
            {
                switch (e.StatusCode)
