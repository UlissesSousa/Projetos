...
namespace Maverick.Application.Tests
{
    public class FilmesServiceTests
    {
        ...
        [Fact]
        [Trait(nameof(IFilmesService.ObterPessoasAsync), "Sucesso")]
        public async Task ObterPessoasAsync_Sucesso()
        {
            // Objeto que sera utilizado para retorno do Mock
            ...
            tmdbAdapterMock
                .Setup(m => m.GetPessoasAsync(It.IsAny<PesquisaPessoas>(), "pt-BR"))
                .ReturnsAsync(expected);

            var pessoa = await filmesService.ObterPessoasAsync(new PesquisaPessoas()
            {
                TermoPesquisa = "teste"
            });
            ...
            Assert.Contains(pessoa, f =>
                    f.Nome == exepectedSingle.Nome);
        }
        [Fact]
        [Trait(nameof(IFilmesService.ObterPessoasAsync), "Erro")]
        public async Task ObterPessoasAsync_Erro()
        {
            await Assert.ThrowsAnyAsync<ModelValidationException>(async () =>
            {
                await filmesService.ObterPessoasAsync(new PesquisaPessoas());
            });
        }
